Shader "Ocean2CrestRemade2"
{

	Properties
	{
		[Header(Normal Mapping)]
		[NoScaleOffset] _MaskTexture("MaskTexture", 2D) = "black" {}
		[Header(Normal Mapping)]
	...
	SubShader
	{
		...
		Tags { "LightMode"="ForwardBase" "Queue"="AlphaTest" "IgnoreProjector"="True" "RenderType"="AlphaTest" }
		...
		Pass
		{
			...
			///////////////////////////////////////////////////
				// Azure Fog: Add the Azure fog scattering data. //
			...
			struct Varyings
			{
				...
				half4 pos2: TEXCOORD10;



					/////////////////////////////////////////
					// Azure Fog: Add Projection TEXCOORD. //
			...
			Varyings Vert(Attributes v)
			{
				...
				o.grabPos = ComputeGrabScreenPos(o.positionCS);
												...
												///////////////////////////////////////////
					// Azure Fog: Compute fog projection uv. //
				o.projPos = o.grabPos;
				COMPUTE_EYEDEPTH(o.projPos.z);
				o.foam_screenPosXYW.yzw = ComputeScreenPos(o.positionCS).xyw;
				o.pos2=ComputeScreenPos(float4(o.worldPos, 1.));
				return o;
			}
			...
			half4 Frag(const Varyings input, const float facing : VFACE) : SV_Target
			{
				...
				if (!underwater)
				{
					// Above water - do atmospheric fog. If you are using a third party sky package such as Azure, replace this with their stuff!
										//////////////////////
					// Apply Azure Fog. //
					col = ApplyAzureFog(float4(col, 1.0), input.grabPos, input.worldPos); // or return ApplyAzureFog(float4(col, 1.0), i.projPos, i.worldPos);
				...
				float2 scrP= input.pos2.xy  + float4(0.5, 0.5, 0, 0);
				float2 texturePos = scrP.xy * _ScreenParams.zw - scrP.xy;
				  texturePos.y = lerp(texturePos.y, 1.0 - texturePos.y, _ProjectionParams.x * 0.5 + 0.5);
				float4 mask= 0.0;
				mask=tex2D(_MaskTexture,uvDepth);
				clip((1.0-mask.r)-0.5f);

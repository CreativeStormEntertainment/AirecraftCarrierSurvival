...
namespace GPUInstancer.CrowdAnimations
{
    public class GPUIMecanimAnimator : GPUIAnimatorWorkflow
    {
        ...
        public GPUIMecanimAnimator(Animator animator)
        {
            activeClipCount = 0;
        ...
        public void UpdateDataFromMecanimAnimator(GPUICrowdRuntimeData runtimeData, int arrayIndex, Animator animatorRef, List<AnimatorClipInfo> animatorClipInfos)
        {
            ...
            for (int i = 0; i < activeClipCount; i++)
            {
                ...
                if (!animationClipData.animationClip.isLooping && stateTime >= 1)
                {
                    runtimeData.animationData[index][i] = animationClipData.clipStartFrame +
                        animationClipData.clipFrameCount - 1;
                }
                else
                {
                    runtimeData.animationData[index][i] = animationClipData.clipStartFrame +
                        (animationClipData.clipFrameCount - 1) * (stateTime % 1.0f);
                }
            }

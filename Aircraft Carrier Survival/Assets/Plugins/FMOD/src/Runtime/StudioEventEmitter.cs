...
namespace FMODUnity
{
    ...
    public class StudioEventEmitter : EventHandler
    {
        ...
        public void Play()
        {
            ...
            if (!instance.isValid())
            {
                //Debug.Log("Created event instance, " + Event);
            ...
            string paramText = "";
            foreach (var paramValue in Params)
            {
                paramText += " " + (int)paramValue.Value + ";";
            }
            ...
            Debug.Log(Time.realtimeSinceStartup.ToString("F3") + " " + Event + paramText);
#endif
            instance.start();

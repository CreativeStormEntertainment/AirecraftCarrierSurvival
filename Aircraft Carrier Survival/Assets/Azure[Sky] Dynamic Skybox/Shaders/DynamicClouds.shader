        ...
        Pass
        {
            CGPROGRAM
                ...
                v2f vertex_program (appdata v)
                {
				    ...
				    // Dynamic cloud position - Old
                    /*float speed = _Azure_DynamicCloudSpeed * _Time;
                    float s = sin(radians(_Azure_DynamicCloudDirection));
                    float c = cos(radians(_Azure_DynamicCloudDirection));
				    
                    float2x2 rotationMatrix = float2x2( c, -s, s, c);
                    Output.CloudPos.y = Output.WorldPos.y * _Azure_DynamicCloudAltitude;
                    Output.CloudPos.xz = mul(Output.WorldPos.xz, rotationMatrix );
                    Output.CloudPos = normalize(Output.CloudPos);
                    Output.CloudUV.xy = Output.CloudPos.xz * 0.25 - 0.005 + float2(speed / 20, speed);
				    Output.CloudUV.zw = Output.CloudPos.xz * 0.35 -0.0065 + float2(speed / 20, speed);*/
                ...
                // Fragment shader
                float4 fragment_program (v2f Input) : SV_Target
                {
                    // Directions

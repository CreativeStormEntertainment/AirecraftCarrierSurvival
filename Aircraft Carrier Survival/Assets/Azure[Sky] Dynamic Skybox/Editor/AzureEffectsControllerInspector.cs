...
namespace UnityEditor.AzureSky
{
	...
	public class AzureEffectsControllerInspector : Editor
	{
        ...
        private readonly GUIContent[] m_guiContent = new[]
        {
			...
			new GUIContent("Heavy Rain Emitter", "The heavy rain emitter."),
		...
		private SerializedProperty m_heavyRainEmitter;
		...
		private void OnEnable()
		{
			...
			m_heavyRainEmitter = serializedObject.FindProperty("heavyRainEmitter");
		...
		public override void OnInspectorGUI()
		{
			...
			if (m_showSoundsHeaderGroup.isExpanded)
			{
				...
				GUI.color = m_greenColor;
				if (!m_target.heavyRainEmitter)
					GUI.color = m_redColor;
				EditorGUILayout.PropertyField(m_heavyRainEmitter, m_guiContent[8]);
			...
			if (m_showParticlesHeaderGroup.isExpanded)
			{
				...
				EditorGUILayout.PropertyField(m_followTarget, m_guiContent[18]);
				
				// Particle system transform
				GUI.color = m_greenColor;
				if (!m_target.particleSystemTransform) GUI.color = m_redColor;
				EditorGUILayout.PropertyField(m_particleSystemTransform, m_guiContent[9]);
				
				// Rain material
				GUI.color = m_greenColor;
				if (!m_target.rainMaterial) GUI.color = m_redColor;
				EditorGUILayout.PropertyField(m_rainMaterial, m_guiContent[10]);
				
				// Heavy rain material
				GUI.color = m_greenColor;
				if (!m_target.heavyRainMaterial) GUI.color = m_redColor;
				EditorGUILayout.PropertyField(m_heavyRainMaterial, m_guiContent[11]);
				
				// Snow material
				GUI.color = m_greenColor;
				if (!m_target.snowMaterial) GUI.color = m_redColor;
				EditorGUILayout.PropertyField(m_snowMaterial, m_guiContent[12]);
				
				// Ripple material
				GUI.color = m_greenColor;
				if (!m_target.rippleMaterial) GUI.color = m_redColor;
				EditorGUILayout.PropertyField(m_rippleMaterial, m_guiContent[13]);
				
				// Light rain particle
				GUI.color = m_greenColor;
				if (!m_target.lightRainParticle) GUI.color = m_redColor;
				EditorGUILayout.PropertyField(m_lightRainParticle, m_guiContent[14]);
				
				// Medium rain particle
				GUI.color = m_greenColor;
				if (!m_target.mediumRainParticle) GUI.color = m_redColor;
				EditorGUILayout.PropertyField(m_mediumRainParticle, m_guiContent[15]);
				
				// Heavy rain particle
				GUI.color = m_greenColor;
				if (!m_target.heavyRainParticle) GUI.color = m_redColor;
				EditorGUILayout.PropertyField(m_heavyRainParticle, m_guiContent[16]);
				
				// Snow particle
				GUI.color = m_greenColor;
				if (!m_target.snowParticle) GUI.color = m_redColor;
				EditorGUILayout.PropertyField(m_snowParticle, m_guiContent[17]);
